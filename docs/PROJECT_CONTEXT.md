...
- The admin `Operations/Orders` DataTable exposes icon-only edit and detail actions for rental orders linked to a rental cohort. The edit action opens the order-period popup in place without navigating to order details. System administrators and operations managers can update the period name plus the order start/end dates; the order period and linked cohort period are saved together and audited. The orders table uses a fixed full-width desktop layout with wrapping cells and hidden horizontal overflow; narrow screens fall back to the shared stacked-card table layout instead of showing a horizontal scrollbar.
