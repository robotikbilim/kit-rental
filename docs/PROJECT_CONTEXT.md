...
- Customer portal `Kits` rows show the actual Kargonomi `Kargo Durumu` label used by the admin order table. Kits without a shipment show `Başlatılmadı`, failed shipments show `Hata`, and the dashboard counts delivered shipments as `Kullanımdaki Kitler`, in-transit shipments as `Gönderimdeki Kitler`, and every other shipment state as `Hazırlanan Kitler`.
