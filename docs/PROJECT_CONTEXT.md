...
- The same admin order-detail table uses a dropdown in the `Kargo Durumu` column filter; it includes `Tümü`, `Başlatılmadı`, `Hata`, and the distinct provider status labels present in the order.
